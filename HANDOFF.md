...
- Phone "zoomed in" bug: `web/index.html` had NO viewport meta (Flutter template omits it) → added `width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no, viewport-fit=cover`.
- Highlight semantics (user request): colour = history, white/gold = options. `GameController.trails` (List<MoveTrail>) keeps every move since the local player last moved; `_TrailPainter` draws each as a glowing line in the mover's seat colour from the start hole (ringed) to the landing hole, moved marbles get a ring in that colour. Selectable marbles glow white; landing spots pulse GOLD (`_TargetMarker` colour). The dotted "option paths" were removed (confusing next to history).
- Bot pace: Settings → "Bot speed" Relaxed/Normal/Fast (`SettingsController.botPace` 1.0/0.55/0.25 stored in prefs). Relaxed = 2.2s think, animation ×2.6 slower, 1.5s pause after each bot/remote move (`GameConfig.botThink/botSlow/botPause`, scaled in `GameController._pace`). Tests that wait for bots must poll (see online_flow_test).
